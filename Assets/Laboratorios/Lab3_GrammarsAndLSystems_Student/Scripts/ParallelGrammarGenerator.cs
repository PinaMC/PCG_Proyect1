...
public class ParallelGrammarGenerator : MonoBehaviour
{
        ...
        List<string> derivation = null)
    {
        ...
        string current = axiom; //current = actual/ahora
        if(derivation != null){ // si se entrega la lista esta se limpia y se agrega el axioma
            derivation.Clear();
            derivation.Add(current);
        }

        for(int i = 0; i< iterations; i++){
            StringBuilder next = new StringBuilder();   // Construir nueva cadena     

            foreach(char symbolChar in current){ //recorrer la nueva cadena
                string simbolo = symbolChar.ToString();
                string reemplazo = null;

                //Buscar el predecesor que coincida con el simbolo

                foreach(LSystemRule rule in rules){
                    if(rule.predecessor == simbolo ){
                        reemplazo = rule.successor;
                        break;
                    }
                }
                // si se encuentra la regla, se añade el sucesor, sino se mantiene
                if(reemplazo != null){
                    next.Append(reemplazo);
                }else{
                    next.Append(symbolChar);
                }
            }
            
            current = next.ToString(); // actualizar cadena
            //registrar la iteracion
            if(derivation != null){
                derivation.Add(current);
            }
        }
    ...
    public void GenerateExpansion()
    {
        ...
        string finalSequence =
            Generate(axiom,rules,iterations,derivation);


        if (showDerivation)
        {
            PrintDerivation(derivation,finalSequence);
        }
    }

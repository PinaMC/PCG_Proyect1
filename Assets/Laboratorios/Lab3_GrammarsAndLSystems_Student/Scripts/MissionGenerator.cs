...
public class MissionGenerator : MonoBehaviour
{
    ...
    public void GenerateMission()
    {
        ...
        List<string> derivation = new List<string>(); //se crea la lista de derivacion 
        derivation.Add(current);

        // se crea un for para la realizacion de pasos secuenciales
        for(int paso = 0; paso < expansionSteps; paso++)
        {
            // ahora hay que  buscar la letra T en el string
            int position = current.IndexOf(taskSymbol);
            if(position == -1){
                // si no se hallo la letra T no hay que seguir avanzando
                Debug.LogWarning(" simbolo no hallado");
                break;
            }
            // ahora se debe de elegir una produccion o tarea al azar
            int indice = random.Next(taskProductions.Count);
            string  production = taskProductions[indice];// asigna a produccion el numero por ej si es 3 sera ktl
            //ahora se reemplaza la t por la produccion que se eligio en el random
            string anterior = current.Substring(0, position);
            string despues = current.Substring(position + taskSymbol.Length); // al sumar toma todo el contenido desde la psocion hacia la derecha o despues de la t
            current = anterior + production + despues;

            // registrar en derivation la accion realizada previamente
            derivation.Add(production);


        } 
        
        // Reamplaza la letra T una ves halla terminado el for por una C
        current = current.Replace(taskSymbol, terminalProduction);
        derivation.Add(current);

        // imprimir cadena 
        //paso a paso
        foreach (string pasos in derivation)
        {
        ...
        //PrintMission(derivation, current); // hace el reporte de muestra para que se vea lo que se hizo
    ...
    // EL OBJETIVO ES MOSTRAR LA INFO Y LAS DERIVACIONES

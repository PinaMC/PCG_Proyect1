...
public class LSystemTreeGenerator : MonoBehaviour
{
        ...
        float activeAngle)
    {
        ...
        Vector3 ActualPosition = Vector3.zero; // (0,0,0) 
        Quaternion ActualRotation = Quaternion.identity; // ↑ Mirando hacia Arriba
        Stack<TurtleState> Tree = new Stack<TurtleState>();// Pila 
        
        // Recorrer los símbolos de sequence e implementar:
        foreach (char ch in sequence)
        {
            Vector3 Direccion = ActualRotation * Vector3.up; // Direccion hacia la que se dirige actualmente;
            switch (ch)
            {
            // 2. Movimiento con y sin dibujo mediante F y f.
                case 'F':
                    Vector3 InicioPosicion = ActualPosition; // Create Branch traza inicio fin con rotacion parte aca
                    
                    
                    ActualPosition = ActualPosition + Direccion * segmentLength; // La posicion a la que se mueve un largo asignado dependiendo hacia donde esta viendo 
                    CreateBranch(InicioPosicion, ActualPosition); // Encargado de dibujar
                    break;
                case 'f': // f se mueve sin dibujar

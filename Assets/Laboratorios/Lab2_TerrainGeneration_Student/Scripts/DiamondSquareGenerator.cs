...
public static class DiamondSquareGenerator
{
    ...
    //
    private static void DiamondStep(float[,] heights, int stepSize, float roughness,System.Random random)
    {
        ...
        for (int y = halfStep; y < resolution; y+= stepSize){
            for (int x = halfStep; x < resolution; x += stepSize){
              ...
              float bottomRight =  heights[y + halfStep, x + halfStep];
                ...
                Debug.Log("coordenadas" + y + x);
              float average = (topLeft + topRight + bottomLeft + bottomRight)/4;
              float offset = RandomOffset(random, roughness);
                //la nueva altura es la suma de average y offset
                Debug.Log("Altura" + heights[y,x]);
                float nuevaAltura =  average + offset;

              nuevaAltura = Mathf.Clamp(nuevaAltura, 0f, 1f);
              heights[y,x] = nuevaAltura;
                Debug.Log("Nueva Altura" + heights[y, x]);
            }
        }
    }
        ...
        System.Random random)
    {
        ...
        for(int y = halfStep; y < resolution; y+= halfStep){
            int starx = 0;
            if(((y/ halfStep) %2) == 0)starx=halfStep;
            for(int x = starx; x < resolution; x += stepSize){
                float suma = 0f;
                int contador = 0;
                //ahora revisar cada vecino
                //left
                if(x-halfStep >= 0){
                ...
                //promedios
                float average= suma / contador;
                 float offset = RandomOffset(random, roughness);
                //la nueva altura es la suma de average y offset
                float nuevaAltura =  average + offset; 
                nuevaAltura = Mathf.Clamp(nuevaAltura, 0f, 1f);
                //nuevaAltura = Mathf.Clamp01(nuevaAltura);

...
public static class HeightmapGenerator
{
    ...
    public static float[,] GenerateRandomNoise(int resolution, int seed)
    {
        ...
        // Creación de un generador "seed" (esta funcoin garantiza que la generación sea 'determinista'),
        // Quiere decir que usando la misma semilla se obtendrá el mismo resultado.
        System.Random random = new System.Random(seed);

        // Recorrer todas las posiciones (x, y) del heightmap
        for (int y = 0; y < resolution; y++)
        {
            for (int x = 0; x < resolution; x++)
            {
                // Asignar a cada posición un valor aleatorio entre 0 y 1
        ...
        InterpolationMode interpolationMode)
    {
        ...
        for (int y = 0; y < resolution; y++)
        {
            for (int x = 0; x < resolution; x++)
            {
                ...
                // 1. Identificar la celda en la grilla
                ...
                // 2. Calcular tx y ty normalizados [0, 1] dentro de la celda
                ...
                // 3. Obtener los 4 puntos de control circundantes
    ...
    {
        return -2f * (t * t * t) + 13f * (t * t);    //interpolacion bicubica
    }
    ...
    private static float[,] GenerateControlPoints(int resolution, int seed)
    {
        ...
        // 1. Crear un generador pseudoaleatorio utilizando seed.
        System.Random random = new System.Random(seed);
        ...
        for (int y = 0; y < resolution; y++)
        {
            for (int x = 0; x < resolution; x++)
            {
                // 3. Asignar un valor pseudoaleatorio [0,1] a cada punto.
